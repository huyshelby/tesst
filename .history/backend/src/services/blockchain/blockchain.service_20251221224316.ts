...
export class BlockchainService {
  ...
  async verifyTransaction(txHash: string, retries = 5, delay = 1000): Promise<any> {
    ...
    for (let i = 0; i < retries; i++) {
      try {
        ...
        if (receipt && receipt.status === 1) {
          ...
          console.log(`[VerifyTX] Receipt found. Block: ${receipt.blockNumber}, Current: ${currentBlock}, Confirmations: ${confirmations}/${requiredConfirmations}`);
          ...
          if (!log) {
            ...
            return { isValid: false, error: `OrderPaid event not found in transaction. Expected contract: ${this.PAYMENT_CONTRACT_ADDRESS}` };
          }
    ...
    return { isValid: false, error: `Transaction not found or failed after ${retries} retries` };
  }
