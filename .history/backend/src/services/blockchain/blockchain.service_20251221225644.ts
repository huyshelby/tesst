      ...
      try {
        ...
        if (receipt) {
          ...
          if (receipt.logs.length === 0) {
            console.error(`[VerifyTX] ❌ Transaction has no logs. This might be a simple ETH transfer, not a contract call.`);
            return {
              isValid: false,
              error: "Transaction has no logs. Please ensure you called the payment contract function (payOrderWithNative or payOrderWithToken), not a simple transfer."
            };
          }
          ...
          if (!log) {
            ...
            // Check if user sent to wrong contract
            const wrongContract = receipt.logs.length > 0 ? receipt.logs[0].address : "unknown";
            return {
              isValid: false,
              error: `OrderPaid event not found. Expected contract: ${this.PAYMENT_CONTRACT_ADDRESS}, but transaction interacted with: ${wrongContract}. Please ensure you're using the correct payment contract.`
            };
          }
          ...
          // Because orderId is indexed in the event, it's hashed and can't be decoded
          // We need to decode it from the transaction input data instead
          ...
          try {
            ...
            console.log(`[VerifyTX] Decoded function call:`, decodedInput.name);
            console.log(`[VerifyTX] Function args:`, decodedInput.args);

            // Get orderId from function arguments (first parameter in both functions)
          ...
          if (!orderId) {
            console.error(`[VerifyTX] ❌ OrderId could not be extracted from transaction`);
            return {
              isValid: false,
              error: "OrderId could not be extracted from transaction. The transaction data may be corrupted."
            };
          }

          if (!parsedLog.args.token) {
            console.error(`[VerifyTX] ❌ Token address is missing in event args`);
            return {
              isValid: false,
              error: "Token address is missing in the OrderPaid event."
            };
          }

          if (!parsedLog.args.amount) {
            console.error(`[VerifyTX] ❌ Amount is missing in event args`);
            return {
              isValid: false,
              error: "Amount is missing in the OrderPaid event."
            };
          }
