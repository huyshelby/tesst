...
export class BlockchainService {
  ...
  async startListening(): Promise<void> {
    ...
    try {
      ...
      this.contract.on("OrderPaid", async (orderId: string, payer: string, amount: any, token: string, paymentMethod: string, timestamp: any, event: any) => {
        console.log[object Object] detected!");
        console.log("[object Object]d);
        console.log("👤 Payer:", payer);
        console.log("[object Object]ers.utils.formatUnits(amount, this.getTokenDecimals(token)));
  ...
  async verifyTransaction(txHash: string): Promise<any> {
    try {
      ...
      if (!log) return { isValid: false, error: "OrderPaid event not found" };
