...
export class BlockchainService {
  ...
  private readonly NFT_RECEIPT_CONTRACT_ADDRESS: string;
  private readonly PAYMENT_CONTRACT_ABI: string[];
  private readonly NFT_RECEIPT_ABI: string[];
  ...
  constructor() {
    ...
    this.NFT_RECEIPT_CONTRACT_ADDRESS = process.env.NFT_RECEIPT_CONTRACT_ADDRESS || "";
    ...
    this.NFT_RECEIPT_ABI = [
      "event OrderReceiptMinted(bytes32 indexed orderHash, uint256 indexed tokenId, address indexed owner, address minter, string metadataUrl)"
    ];
  ...
  private async verifyNFTReceiptTransaction(txHash: string, orderId: string): Promise<any> {
    const receipt = await this.provider.getTransactionReceipt(txHash);
    if (!receipt) {
      return { isValid: false, error: "Transaction receipt not found" };
    }

    const iface = new ethers.utils.Interface(this.NFT_RECEIPT_ABI);
    const eventTopic = iface.getEventTopic('OrderReceiptMinted');
    ...
    for (const log of receipt.logs) {
      if (log.topics[0] === eventTopic) {
        try {
          ...
          // Tạo lại orderHash từ orderId để so sánh
          const expectedOrderHash = ethers.utils.keccak256(
            ethers.utils.defaultAbiCoder.encode(['string'], [orderId])
          );
          ...
          if (orderHash === expectedOrderHash) {
            return {
              ...
              metadataUrl: parsedLog.args.metadataUrl
            };
          }
        } catch (error) {
          console.error('Error parsing NFT receipt event:', error);
        }
      }
    }

    return { isValid: false, error: "No matching NFT receipt minted event found" };
  }
 ...
 public async processPayment(orderId: string, txHash: string): Promise<any> {
  try {
    ...
    // Thử kiểm tra giao dịch NFT receipt
    const nftVerification = await this.verifyNFTReceiptTransaction(txHash, orderId);
    if (nftVerification.isValid) {
      console.log(`[ProcessPayment] ✅ NFT Receipt minted:`, nftVerification);
      return {
        status: 'completed',
        type: 'nft_receipt',
        data: nftVerification
      };
    }

    // Nếu không phải giao dịch NFT, kiểm tra giao dịch thanh toán thông thường
    const paymentVerification = await this.verifyTransaction(txHash);
    if (!paymentVerification.isValid) {
      throw new Error(paymentVerification.error || "Transaction verification failed");
    }
