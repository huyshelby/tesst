    ...
    for (let i = 0; i < retries; i++) {
      try {
        ...
        if (receipt) {
          ...
          if (!parsedLog.args.orderId) {
            console.error(`[VerifyTX] ❌ OrderId is missing in event args`);
            return {
              isValid: false,
              error: "OrderId is missing in the OrderPaid event. The event data may be corrupted."
            };
          }
          ...
          console.log(`[VerifyTX] ✅ Event validated. OrderId: ${parsedLog.args.orderId}`);
