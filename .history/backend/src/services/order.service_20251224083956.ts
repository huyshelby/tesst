...
export class OrderService {
  ...
  static async mintOrderReceipt(orderId: string, userId: string) {
    ...
    try {
      ...
      // Update order with NFT info
      await prisma.order.update({
        where: { id: orderId },
        data: {
          nftTokenId: result.tokenId,
          nftMintTxHash: result.txHash,
      ...
      console.error('Error in mintOrderReceipt:', error);
      throw new Error(`Failed to mint NFT receipt: ${error.message}`);
    }
  }
  ...
  static async getOrderReceipt(orderId: string, userId: string) {
    ...
    if (order.userId !== userId) {
      ...
      if (user?.role !== 'ADMIN') {
        throw new Error('Unauthorized to view this receipt');
      }
    }
    ...
    try {
      ...
      return {
        ...
        mintedAt: order.nftMintedAt,
        metadataUrl: order.nftMetadataUrl,
      ...
      return {
        ...
        mintedAt: order.nftMintedAt,
        metadataUrl: order.nftMetadataUrl,
        error: 'Failed to load receipt metadata'
      };
    }
  }
