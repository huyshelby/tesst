...
const categoryConfig = {
  title: "Apple Watch",
  description: "Đồng hồ thông minh theo dõi sức khỏe và phong cách sống",
  filters: ["Tất cả", "Ultra", "Series", "SE"],
  categoryMatch: "watch",
};
...
export default function WatchPage() {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const rams = params.getAll("ram").map(Number).filter((n) => !isNaN(n));
        const storages = params.getAll("storage").map(Number).filter((n) => !isNaN(n));
        const priceId = params.get("price") ?? undefined;
        const priceRange = FILTERS.priceRange.find((p) => p.id === priceId || undefined);
        const sort = params.get("sort");
        const sortByParam = sort === "priceAsc" || sort === "priceDesc" ? ("price" as const) : sort === "rating" ? ("rating" as const) : ("createdAt" as const);
        const order = sort === "priceAsc" ? ("asc" as const) : ("desc" as const);
        ...
        if (mounted) setItems(Array.isArray(res.products) ? res.products : []);
      } catch (e) {
        console.error(e);
      } finally {
  ...
  let filteredProducts: Product[] = Array.isArray(items) ? items : [];
        ...
        <div className="content-container max-w-[1280px] py-8">
          <div className="grid grid-cols-12 gap-6">
            <aside className="hidden md:block md:col-span-3 lg:col-span-3">
              <Filters category={"watch" as CategoryKey} />
            </aside>
