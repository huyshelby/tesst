...
const productSchema = z.object({
  ...
  image: z.union([z.string().min(1, 'Ảnh đại diện là bắt buộc'), z.instanceof(File)]),
  ...
  const onSubmit = async (data: ProductFormData) => {
    try {
      ...
      if (data.image instanceof File) {
        ...
        data.image = result.url // Replace File with uploaded URL
        
        // Update form value để component nhận URL mới
      ...
      updateMutation.mutate(
        { id: productId, data },
        {
          onSuccess: async (updatedProduct) => {
            ...
            // Refetch product mới từ server để có data đầy đủ
            await queryClient.invalidateQueries({ queryKey: ['products', productId] })

            // Reset form với data đã upload (với URL mới)
            form.reset({
              name: data.name,
              slug: data.slug,
              ...
              image: data.image, // Đã là string URL sau khi upload
              brand: data.brand,
              stock: data.stock,
            ...
            toast.success('Đã lưu và cập nhật giao diện')
          },
        }
      )
    } catch (error: any) {
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription className="text-xs">
                          Chọn ảnh mới để thay thế. Ảnh sẽ được upload khi bạn nhấn "Lưu thay đổi".
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
