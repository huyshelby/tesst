...
export function ImageUpload({ value, onChange, disabled }: ImageUploadProps) {
  ...
  useEffect(() => {
    if (typeof value === 'string' && value !== preview) {
      console.log('ImageUpload: Updating preview to', value)
      setPreview(value)
      // Reset selectedFile when value is a URL (after successful upload)
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => {
      ...
      console.log('ImageUpload: Preview set to base64 data URL')
    }
    ...
    toast.success('Đã chọn ảnh, nhấn "Lưu sản phẩm" để upload')
  }
      ...
      {preview ? (
        <div className="relative border-2 border-dashed rounded-lg p-4 bg-gray-50">
          <div className="relative w-full aspect-square max-w-xs mx-auto bg-white rounded border overflow-hidden">
            <img
              src={
                typeof preview === 'string' && preview.startsWith('http') || typeof preview === 'string' && preview.startsWith('https')
                  ? preview
                  : typeof preview === 'string' && preview.startsWith('data:')
                    ? preview
                    : typeof preview === 'string' && (preview.startsWith('/pictures') || preview.startsWith('/uploads'))
