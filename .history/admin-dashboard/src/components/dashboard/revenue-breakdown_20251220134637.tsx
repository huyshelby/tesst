    ...
    <div className="bg-white rounded-lg shadow border border-gray-200 p-6">
          ...
          return (
            ...
            >
              ...
              <p className={`text-2xl font-bold ${colors.text} ${metric.highlight ? 'text-3xl' : ''}`}>
                {metric.isNegative ? '-' : ''}
                {formatCurrencyCompactVN(metric.value)}
              </p>
      ...
      <div className="mt-6 pt-6 border-t border-gray-200">
        ...
        <div className="text-center mt-3 text-xs text-gray-600">
          {formatCurrencyCompactVN(revenue.subtotal)} + {formatCurrencyCompactVN(revenue.shippingFee)} - {formatCurrencyCompactVN(revenue.discount)} = <strong className="text-purple-600">{formatCurrencyCompactVN(revenue.total)}</strong>
        </div>
      </div>
    </div>
  )
