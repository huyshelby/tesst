...
// Format image URL to absolute backend origin (strip trailing /api if present)
export function formatImageUrl(url: string | null | undefined): string {
  if (!url) return ''
  if (/^(https?:)?\/\//.test(url) || url.startsWith('blob:') || url.startsWith('data:')) return url

  const raw = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'
  // Derive origin and remove trailing /api if provided
  let origin = raw
  try {
    const u = new URL(raw)
    origin = u.origin
  ...
  const joined = `${origin.replace(/\/$/, '')}/${url.replace(/^\//, '')}`
  return joined
}
